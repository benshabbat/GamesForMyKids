...
> ראו את הסקיל `.claude/skills/game-creation/SKILL.md` — הוא מקור האמת הטכני ומתעדכן יחד עם הקוד.
...
משחקי חידון (Style B/C/E) לא מכוסים במדריך זה בפירוט — יש להם שלושה תת-סגנונות (נתונים בלבד / factory מותאם / רכיב עצמאי מורכב) עם קבצי רישום נפרדים תחת `lib/quiz/registry/`. ראו "Style B/C/E" בסקיל `.claude/skills/game-creation/SKILL.md` לצ'קליסט המלא והעדכני.
