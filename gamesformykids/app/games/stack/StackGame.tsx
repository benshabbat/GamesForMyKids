...
const FLOOR_Y = H - 40; // y of floor block top
const INIT_W  = 170;
const COLORS  = ['#ef4444','#f97316','#eab308','#22c55e','#06b6d4','#3b82f6','#8b5cf6','#ec4899'];
const TARGET_TOP = 90;  // desired screen-y for tower top
...
export default function StackGame() {
  ...
  const st = useRef({
    ...
    curDir: 1 as 1 | -1,
    curSpeed: 2.5,
    camOffset: 0, // added to block.y when drawing (shifts blocks down for camera)
    score: 0, best: 0,
    colorIdx: 0,
    frame: 0, raf: 0,
  });
  ...
  const drop = useCallback(() => {
    ...
    // current slider screen-y → world-y
    ...
    // Camera: keep tower top near TARGET_TOP on screen
    const topWorldY = sliderWorldY; // topmost block y
  ...
  useEffect(() => {
    ...
    function loop() {
      ...
      if (s.phase === 'playing') {
        ...
        const top = s.blocks[s.blocks.length - 1];
        // bounce off screen edges with some overshoot
        ...
        // silence unused warning about top
        void top;
      }
