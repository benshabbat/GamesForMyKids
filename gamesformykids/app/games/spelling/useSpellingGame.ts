...
import { SPELLING_WORDS, SpellingWord, QUESTIONS_PER_GAME } from './data/words';
...
export function useSpellingGame() {
  ...
  const [questions, setQuestions] = useState<SpellingWord[]>([]);
  ...
  const startGame = useCallback(() => {
    const qs = shuffle(SPELLING_WORDS).slice(0, QUESTIONS_PER_GAME);
    setQuestions(qs);
    setIndex(0);
    setScore(0);
    setSelected(null);
  ...
  const choices: string[] = current
    ? shuffle([current.word, ...current.wrong])
    : [];
