...
export default function HumanBodyQuestion({ phase, currentIndex, total, score, currentQuestion, choices, selected, isCorrect, onSelect, onNext, onMenu }: Props) {
  return (
      ...
      <div className="flex-1 flex flex-col items-center justify-center">
        ...
        <div className="grid grid-cols-1 gap-3 w-full max-w-md">
          {choices.map(choice => {
            ...
            return (
              <button key={choice} onClick={() => onSelect(choice)} disabled={phase === 'answered'} className={cls}>
                {choice}
              </button>
            );
          })}
