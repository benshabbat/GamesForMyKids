...
import { CAPITAL_QUESTIONS, QUESTIONS_PER_GAME, type CapitalQuestion } from './data/capitals';
...
function makeChoices(correct: CapitalQuestion): string[] {
  return shuffle([correct.capital, ...correct.wrongOptions]);
}
...
export function useCapitalsGame() {
  ...
  const startGame = useCallback(() => {
    const q = shuffle(CAPITAL_QUESTIONS).slice(0, QUESTIONS_PER_GAME);
    setQuestions(q);
    setIndex(0);
    setScore(0);
    setSelected(null);
  ...
  const selectAnswer = useCallback((capital: string) => {
